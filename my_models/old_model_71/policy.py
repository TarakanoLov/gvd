...
def make_model_79_5():
    n_inputs = 2 + 2 + 6 + 6 # только данные игроков, без учета карт
    
    board_input = Input(shape=(n_inputs,), name='board_input')
    batch_normalization = BatchNormalization()(board_input) # в те времена я не знал как нормально сделать нормализацию входных данных, в принципе сейчас тоже
    
    action_layer1 = Dense(512, activation='relu')(batch_normalization) # кажется, из-за того, что слой один он может представлять только линейную почти комбинацию, раньше я не знал, что нужен еще один слой, чтобы нормально все делать

    
    action_input = Input(shape=(34 * 3 * 2,), name='action_input') # карты которые есть в руке, чтобы для них подсчитать результат, и при обучение, фактически карта которая играл = 1, а остальные 0, чтобы обучать только один конкретный выход
    ...
    opt = SGD(lr=0.00005) # в те времена я не знал, что Adam строго лучше
...
class Agent(Player):
    ...
    def nextCard(self, other, only_drop=False):
        if not Agent.is_load:
            ...
            Agent.model = make_model_79_5()
            Agent.model.load_weights('my_models/old_model_71/variants/model_80.h5') # ебанный блять в рот нахуй я не понимаю блять как сделать чтобы по относительному пути блять подгружать хуйню эту блять
            
        action_val = Agent.model.predict([gen_array_input(self, other), gen_my_all_card(self)])[0]
